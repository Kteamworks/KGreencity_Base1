	...
	if($_REQUEST['radio_type_of_payment']=='pre_payment')
	 {
		 ...
		 frontPayment($form_pid, $encounter, $form_method, $form_source, $_REQUEST['form_prepayment'], 0, $dop,$form_towards);//insertion to 'payments' table.
...
//----------------------------------------------------------------------------------------------------
			if($_REQUEST['radio_type_of_payment']=='copay')//copay saving to ar_session and ar_activity tables
			 {
				$session_id=idSqlStatement("INSERT INTO ar_session (payer_id,user_id,reference,check_date,deposit_date,pay_total,".
				 ...
				 frontPayment($form_pid, $enc, $form_method, $form_source, $amount, 0, $dop,$form_towards);//insertion to 'payments' table.
			 }
	...
	//--------------------------------------------------------------------------------------------------------------------
        			...
        			frontPayment($form_pid, $enc, $form_method, $form_source, 0, $amount, $dop,$form_towards);//insertion to 'payments' table.
...
if ($_POST['form_save'] || $_REQUEST['receipt']) {
  ...
  $payrow = sqlQuery("SELECT " .
    ...
    "FROM payments WHERE " .
    "pid = ? AND dtime = ?", array($form_pid,$dop) );
