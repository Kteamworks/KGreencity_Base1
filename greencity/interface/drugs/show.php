...
if (isset($_POST['submit'])) {
	...
	/*$list = sqlStatement("SELECT b.date,b.fee, b.pid, b.encounter, b.code_type, b.code, b.units,
		...
		*/
		$list = sqlStatement("select distinct b.bill_id,d.discharge_date dc, fe.* from form_encounter fe join billing b
        on fe.encounter=b.encounter
        join t_form_admit d
        on  fe.encounter=d.encounter
        where d.discharge_date>='$from 00:00:00' and d.discharge_date<='$to 23:59:59' and substring(encounter_ipop,1,2) = '$cat'
...
<form method="post" action="">
    ...
    <div class="row">
		...
		<tr><th>From</th><th>To</th><th>Category</th><th></th><th></th><tr>
		...
		<td><button class="form-control" onclick="myFunction()">Print</button></td>
        <script>
        function myFunction() {
        window.print();
        }
...
<form method="post" action="">
			...
			?>
					...
					<td>
                           <input type='text' name='patient[]' value='<?php echo $patient['genericname1']; ?>' style="height:2em;border:1px solid white;" readonly>
					 ...
					 <td>
                        <input type='text' name='cat[]' value='<?php echo $list1['dc'];   ?>' style="height:2em;border:1px solid white;" readonly>
 </td>
  <td>
